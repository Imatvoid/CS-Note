...
正因此特性，我们可以很容易的通过 SPI 机制为我们的程序提供拓展功能。SPI 机制在第三方框架中也有所应用，比如 Dubbo 就是通过 SPI 机制加载所有的组件。

## Java SPI

### 使用

META-INF/services/以接口全限定名命名的文件

```java
com.learn.spi.java.impl.OptimusPrime
com.learn.spi.java.impl.Bumblebee
```
...
- 算是使用的延迟加载,但使用循环,所有实现类都会被加载.不支持按需加载
...
META-INF/dubbo/以接口全限定名命名的文件

```java
optimusPrime = com.learn.spi.java.impl.OptimusPrime
bumblebee = com.learn.spi.java.impl.Bumblebee
```
...
其中getExtensionLoader 方法用于从缓存中获取与拓展类对应的 ExtensionLoader，若缓存未命中，则创建一个新的实例。
